...
- [ ] **0-3** 로컬 레포 클론 + 코드 push
  ```bash
  git clone https://gitlab.com/gitlab-ai-hackathon/{namespace}/duo-agentflow-auditor.git
  # duo-agentflow-auditor/ 폴더 전체 내용 복사
  git add -A && git commit -m "feat: initial project setup" && git push
  ```
...
- [ ] **2A-1** 2-에이전트 축소 플로우 YAML 작성
  - `.gitlab/duo/flows/security-audit.yaml`에서 fix_agent, metrics_agent 주석 처리
...
- [ ] **3B-3** Metrics 플로우 통합
  ...
  - 📝 **문서 업데이트**: 최종 동작하는 YAML을 `.gitlab/duo/flows/security-audit.yaml`에 반영
